...
class ModelLoader:
    """A utility class to load and manage embedding models and LLM models."""
    ...
    def load_llm(self):
        """Load and return the LLM Model dynamically based on the provider in config."""
        llm_block = self.config["llm"]
        # Default provider or choose from env
        provider_key = os.getenv("LLM_PROVIDER", "groq") #Default Groq

        if provider_key not in llm_block:
            log.error("LLM provider not found in config", provider_key=provider_key)
            raise ValueError(f"LLM provider '{provider_key}' not found in config", sys)
